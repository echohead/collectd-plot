...
describe 'the service' do
  ...
  context 'when running on a persistence shard' do
    ...
    it 'should return the rrd file for a given metric' do
      actual = get('/hosts/host_a/plugin/cpu/instance/0/rrd/cpu-user').body
  ...
  context 'when acting as a proxy' do
    ...
    it 'should retrieve rrd files from the appropriate shard for a host' do
      ...
      CollectdPlot::RRDRemote.stub!(:http_get).with("192.168.50.16/hosts/baz/plugin/cpu/instance/0/rrd/cpu-idle").and_return(rrd_data)
      get_json("/hosts/baz/plugin/cpu/instance/0/rrd/cpu-idle").body.should == rrd_data
    end
